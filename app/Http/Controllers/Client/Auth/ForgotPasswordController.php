...
use App\Notifications\ForgotClientPassword;
...
class ForgotPasswordController extends Controller
{
    ...
    */
    ...
    public function verify(Request $request){
        ...
        $admin_email = Customer::where('email',$request->email)->first();

        if ($admin_email) {
            $admin_email->notify(new ForgotClientPassword());
    ...
    public function confirm($id,$email){
        $admin_confirm = Customer::where('id',$id)->where('email',$email)->first();
        $token= str_replace('/','',Hash::make(Str::random(40)));
        if($admin_confirm){
            return view('client.auth.passwords.reset',compact('admin_confirm','id','token','email'));
        }
    }
    ...
    public function update(Request $request,$email){
        ...
        $token = $request->token;
        if ($token) {
            $update_admin_email = Customer::where('email',$email)->first();
            
            if ($update_admin_email) {
                $update_admin_email->update(['password' => Hash::make($request->password)]);
                return redirect()->route('customer.customer.login')->with('message','Votre mot de passe a ete modifier avec success,veuillez vous connecter a nouveu');
            }else {
